...
class ResNet18(torch.nn.Module):
	def __init__(self, num_classes: int = 1000):
		...
		# Adapt for 32x32: smaller first conv, no aggressive downsampling
...
def evaluate_accuracy(model, loader) -> float:
	model.eval()
	try:  # infer device from model params
...
def train_model(
	...
	trn, vld = make_dataloader(
		dataset=cfg.dataset,
		batch_size=cfg.batch_size,
		device=device if cfg.data_on_device else 'cpu',
	)
...
def plot_results(
	...
	for name, d in results.items():
		...
		# single run: (T,)
		if loss.ndim == 1:
			y = smooth_1d(loss[intvl], smooth)
			ax_loss.plot(y, label=name, alpha=0.85)
		# multi-seed: (S, T)
		else:
			y = np.stack([smooth_1d(v[intvl], smooth) for v in loss], axis=0)
		...
		if acc.ndim == 1:
			y = smooth_1d(acc[intvl], smooth)
			ax_acc.plot(y, label=name, lw=1.0, marker="o", markersize=3)
		else:
			y = np.stack([smooth_1d(v[intvl], smooth) for v in acc], axis=0)
			mu, sd = y.mean(axis=0), y.std(axis=0)
